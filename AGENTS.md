...
本文件是 Codex 默认自动加载的项目指令文件。后续如果用户纠正了项目约定、动画规范、命题列表、渲染方式或代码组织规则，应同步更新 `AGENTS.md`，让后续 agent 会话继承最新规范。

当完成一个新命题、修改旧命题规则，或发现本文件与实际项目不一致时，应优先更新本文件中的对应说明，避免继续维护历史文件名或重复的项目指令文件。
...
- **命题1.24**: 两三角形中两边对应相等，夹角大者所对的边也较大
- **命题1.25**: 两三角形中两边对应相等，第三边较大者所对的角也较大
- **命题1.26**: 两三角形中两角对应相等，且一边对应相等，则其余边角也对应相等
- **命题1.27**: 一条直线与两条直线相交，若内错角相等，则两直线平行
- **命题1.28**: 一条直线与两条直线相交，若同位角相等或同旁内角之和等于两直角，则两直线平行
...
- **定义1.2 (线)**: 线是没有宽的长
- **定义1.3**: 线之端是点
- **定义1.4 (直线)**: 直线是其上均匀放置着点的线
- **定义1.5 (面)**: 面是只有长和宽的东西
- **定义1.6**: 面之端是线
- **定义1.7 (平面)**: 平面是其上均匀放置着直线的面
- **定义1.8 (平面角)**: 一个平面上两条线之间的倾斜，它们相交且不在一条直线上
- **定义1.9 (直线角)**: 当夹这个角的线是直线时，这个角叫作直线角
- **定义1.10 (直角、垂直)**: 当一条直线与另一条直线交成的邻角彼此相等时，这些等角中的每一个都是直角，且称这条直线垂直于另一条直线
- **定义1.11 (钝角)**: 钝角是大于直角的角
- **定义1.12 (锐角)**: 锐角是小于直角的角
- **定义1.13 (边界)**: 边界是某个东西的端
- **定义1.14 (形)**: 形是由某一边界或若干边界所围成的东西
- **定义1.15 (圆)**: 由一条线所围成的平面形，其内有一点与这条线上的点连成的所有线段都相等
- **定义1.16 (圆心)**: 上述定义中的"一点"叫作圆心
- **定义1.17 (直径)**: 圆的直径是任意一条过圆心作出且沿两个方向被圆周截得的直线，且该直线把圆二等分
- **定义1.18 (半圆)**: 半圆是由直径和它截得的圆周所围成的图形；半圆的心和圆心相同
- **定义1.19 (直线形、三边形、四边形、多边形)**: 直线形是由直线围成的形——三边形由三条直线围成，四边形由四条直线围成，多边形由四条以上直线围成
- **定义1.20 (等边/等腰/不等边三角形)**: 三边形中，三边均相等的叫作等边三角形，只有两边相等的叫作等腰三角形，三边各不相等的叫作不等边三角形
- **定义1.21 (直角/钝角/锐角三角形)**: 三边形中，有一个直角的叫作直角三角形，有一个钝角的叫作钝角三角形，三个角均为锐角的叫作锐角三角形
- **定义1.22 (正方形/长方形/菱形/长菱形/不规则四边形)**: 四边形中，等边且均为直角的叫作正方形，均为直角但不等边的叫作长方形，等边但非直角的叫作菱形，对角对边相等但不等边且非直角的叫作长菱形，其他四边形叫作不规则四边形
- **定义1.23 (平行直线)**: 同一平面上沿两个方向无定限延长、不论沿哪个方向都不相交的两条直线
